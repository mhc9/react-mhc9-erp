...
const NavDropdownMenu = ({ isShow, hide, submenus, userRole, currentPath }) => {
    return (
            ...
            {submenus.map((menu, index) => {
                ...
                if (menu.type === 'group') {
                    const hasAllowedChildren = menu.submenus.some(child => 
                        ...
                        (child.allowed_users.length === 0 || child.allowed_users.includes(userRole))
                    );

                    if (!hasAllowedChildren) return null;

                    const isGroupActive = menu.submenus.some(child => currentPath && matchPath(currentPath, child.link));
                        ...
                        <li key={`grp-${index}`} className={`relative group/nested hover:bg-gray-100 ${isGroupActive ? 'bg-blue-50' : ''}`}>
                            ...
                            <ul className="absolute left-[98%] top-0 hidden group-hover/nested:block w-52 bg-white border border-gray-100 shadow-[0_10px_25px_-5px_rgba(0,0,0,0.1),0_8px_10px_-6px_rgba(0,0,0,0.1)] rounded-lg z-[60] py-1">
                                {menu.submenus.map((child, cIndex) => {
                                    const isChildAllowed = typeof userRole !== 'undefined' && (child.allowed_users.length === 0 || child.allowed_users.includes(userRole));
                                    if (!isChildAllowed) return null;
                                    const isActive = currentPath && matchPath(currentPath, child.link);
                                    return (
                ...
                // Regular menu item
                const isAllowed = typeof userRole !== 'undefined' && (menu.allowed_users.length === 0 || menu.allowed_users.includes(userRole));
